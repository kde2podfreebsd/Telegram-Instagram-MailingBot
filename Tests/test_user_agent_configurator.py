...
import json
import pytest
from App.UserAgent.Configurator import UserAgentConfigurator, ChannelConfig, UserAgentConfig, ConfigValidationException
...
def test_load_config(configurator):
    ...
    loaded_config = configurator.load_config()
...
def test_validate_channels_config(configurator):
    ...
    invalid_channels_config = [
        ChannelConfig(
            ...
            advertising_messages=["Invalid Message"],  # Непустое, но недопустимое сообщение
        )
    ]
