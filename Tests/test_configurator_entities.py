...
from App.UserAgent.Configurator.ConfiguratorEntities import AppConfig
from App.UserAgent.Configurator.ConfiguratorEntities import ChannelConfig
from App.UserAgent.Configurator.ConfiguratorEntities import ConfigValidationException
from App.UserAgent.Configurator.ConfiguratorEntities import UserAgentConfig


# Тесты для класса ChannelConfig
def test_channel_config_creation():
    config = ChannelConfig(
        "channel1", ["chat1", "chat2"], ["account1"], ["message1", "message2"]
    )
...
def test_user_agent_config_to_dict():
    config = UserAgentConfig("param1", "param2", "param3")
    expected_dict = {"param1": "param1", "param2": "param2", "param3": "param3"}
    assert config.to_dict() == expected_dict
...
def test_config_validation_exception():
    ...
    assert (
        str(exception) == "Ошибка конфигурации: Поле 'field1' в индексе 0 недопустимо."
    )
